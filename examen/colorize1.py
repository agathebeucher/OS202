...
def create_field( values, ifield, nb_layers, prolong_field = False ):
    ...
    le ifield ieme champs qu'on stocke dans un nouveau tableau auquel on rajoute
...
if __name__ == '__main__':
    if rank == 0:
        # Le processus 0 (maître) charge/lie l'image et la distribue
        im_gray = Image.open(gray_img)
        im_gray = im_gray.convert('HSV')
        values_gray = np.array(im_gray)
        # On sépare l'image en tranches verticales(axis=1)en fonction du nombre de processeur 
        slices_values_gray = np.array_split(values_gray, size, axis=1)

        # Distribution du calcul des seconds membres
        im = Image.open(marked_img)
        im_ycbcr = im.convert('YCbCr')
        val_ycbcr = np.array(im_ycbcr)
        slices_val_ycbcr=np.array_split(val_ycbcr,size,axis=1)
        
        # Distribution 
        im_hsv = im.convert("HSV")
        val_hsv = np.array(im_hsv)
        slices_val_hsv=np.array_split(val_hsv,size,axis=1)
    else:
        slices_values_gray = None
        slices_val_hsv = None
        slices_val_ycbcr = None

    # Répartition des tranches de manière équitable à tous les processus (y compris lui-même)
    slice_values_gray = comm.scatter(slices_values_gray, root=0)
    slice_val_ycbcr = comm.scatter(slices_val_ycbcr, root=0)
    
    slice_values_gray=np.array(slice_values_gray)
    slice_val_ycbcr=np.array(slice_val_ycbcr)

    slice_val_hsv=comm.scatter(slices_val_hsv, root=0)
    slice_val_hsv=np.array(slice_val_hsv)
    
    # Dans chaque processus :
    # On crée le tableau d'intensite en rajoutant deux couches de cellules fantomes dans chaque direction :
    intensity_local = (1./255.)*create_field(slice_values_gray, INTENSITY, nb_layers=2, prolong_field=True)
    ...
    deb = time.time()
    variance = compute_variance(intensity_local, means_local)
    end = time.time() - deb
    print(f"Temps calcul variance : {end} secondes")
    # Calcul de la matrice utilisee pour minimiser la fonction quadratique
    deb = time.time()
    A = compute_matrix((means_local.shape[1]-2,means_local.shape[0]-2), 0, intensity_local, means_local, variance)
    end = time.time() - deb
    print(f"Temps calcul matrice : {end} secondes")

        # Les composantes Cb (bleu) et Cr (Rouge) sont normalisees :
    Cb = (1./255.)*np.array(slice_val_ycbcr[:,:,CB].flat, dtype=np.double)
    Cr = (1./255.)*np.array(slice_val_ycbcr[:,:,CR].flat, dtype=np.double)
    ...
    deb = time.time()
    fix_coul_indices = search_fixed_colored_pixels(slice_val_hsv)
    end = time.time() - deb
    ...
    processed_slice = np.empty((shape[0],shape[1],3), dtype=np.uint8)
    processed_slice[:,:,0] = intensity_local[2:-2,2:-2].astype('uint8')
    processed_slice[:,:,1] = np.reshape(new_Cb, shape).astype('uint8')
    processed_slice[:,:,2] = np.reshape(new_Cr, shape).astype('uint8')

    # Rassembler les tranches traitées dans le processus maître
    processed_slices = comm.gather(processed_slice, root=0)

    if rank == 0:
        # Reconstruire l'image à partir des tranches traitées
        processed_slices=np.array(processed_slices)
        reconstructed_image = np.concatenate(processed_slices, axis=1)
        # Sauvegarder l'image résultante
        output_image = Image.fromarray(reconstructed_image, mode='YCbCr')
        output_image.convert('RGB').save(output, 'PNG')
