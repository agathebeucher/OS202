# Produit matrice-vecteur v = A.u par COLONNE
import numpy as np
...
local_A = np.array([[(i + j) % dim + 1. for j in range(rank*N_loc, rank*N_loc + N_loc)] for i in range(dim)])

print(f"local_A = {local_A} pour le processeur {rank}")

# Initialisation du vecteur u_local
#u = np.array([i+1. for i in range(dim)])
local_u = np.array([(rank * N_loc + i) + 1. for i in range(N_loc)])
print(f"u = {local_u}")
