...
class Colony:
    ...
    def explore(self, unloaded_ants, the_maze, pos_food, pos_nest, pheromones):
        ...
        has_north_exit = np.bitwise_and(the_maze.maze[old_pos_ants[:, 0], old_pos_ants[:, 1]], maze_sequentiel.NORTH) > 0
        has_east_exit = np.bitwise_and(the_maze.maze[old_pos_ants[:, 0], old_pos_ants[:, 1]], maze_sequentiel.EAST) > 0
        has_south_exit = np.bitwise_and(the_maze.maze[old_pos_ants[:, 0], old_pos_ants[:, 1]], maze_sequentiel.SOUTH) > 0
        has_west_exit = np.bitwise_and(the_maze.maze[old_pos_ants[:, 0], old_pos_ants[:, 1]], maze_sequentiel.WEST) > 0
...
if __name__ == "__main__":
    ...
    pos_nest = 0, 0
    a_maze = maze_sequentiel.Maze(size_laby, 12345)
