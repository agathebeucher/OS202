...
"""
OBJECTIF : ici on parallélise le code du jeu en trois processeurs et on divise la grille verticalement en deux parties: 
    - P0 : traite l'affichage des résultats et rassemble les calculs
    - P1 : traite les calculs pour la partie supérieur
    - P2 : traite les calculs pour la partie inférieure
"""
import pygame as pg
import numpy as np
...
if __name__ == '__main__':
    ...
    if rank==0:
        appli = App((resx, resy), grid) #Affichage de l'interface
    ny, nx = grid.dimensions
    
    #Diviser la grille en deux parties pour chaque processeur
    local_grid = Grille((ny//2, nx), color_life=grid.col_life, color_dead=grid.col_dead)
    if rank==1:
        ny = ny // 2
        local_grid.cells = grid.cells[(rank - 1)*ny:rank*ny, :]
    if rank==2:
        ny = ny - (ny // 2)
        local_grid.cells = grid.cells[(rank - 1)*ny:rank*ny, :]
    while True:
        ...
        # Calcul d'une partie de la grille pour P1 et P2
        if rank!=0:
            # Calculer la prochaine génération
            diff_cells = local_grid.compute_next_iteration()
            t2 = time.time()
        

        # Rassembler les résultats des calculs de chaque processeur vers P0
        diff_cells_list = comm.gather(diff_cells, root=0)
        t2_list=comm.gather(t2,root=0)
        # Affichage sur le processeur 0
        if rank == 0:
            # Mettre à jour la grille avec les différences reçues
            decalage=0
            for diff in diff_cells_list:
                for cell_index in diff:
                        if diff==diff_cells_list[2]:
                            decalage=ny
                        i = decalage+cell_index // grid.dimensions[1]
                        j = decalage+cell_index % grid.dimensions[1]
                        grid.cells[i, j] = 1 - grid.cells[i, j]  # Inverser l'état
            appli.draw()
            t3 = time.time()
