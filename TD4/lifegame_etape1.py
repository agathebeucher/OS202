...
"""
"""
OBJECTIF : ici on parallélise le code du jeu en deux processeur : 
    - P0 : traite l'affichage des résultats et rassemble les calculs
    - P1 : traite les calculs
"""
import pygame as pg
import numpy as np
...
if __name__ == '__main__':
    ...
    #Procédure pour l'affichage sur le processeur 0 (P0)
    while True:
        # Fonction pour le calcul sur le processeur 1 (P1)):
        if rank!=0:
            t1 = time.time()
            diff = grid.compute_next_iteration()
            sd=[diff,t1]
            comm.send(sd, dest=0)  # Envoyer les différences à P0 pour l'affichage
        
        # Affichage sur le processeur 0
        if rank==0:
            [diff, t1] = comm.recv(source=1)
            t2 = time.time()
            # Mettre à jour la grille avec les différences reçues
            for cell_index in diff:
                # Convertir l'index de la cellule en coordonnées (i, j)
                i = cell_index // grid.dimensions[1]
                j = cell_index % grid.dimensions[1]
                # Mettre à jour l'état de la cellule dans la grille
                grid.cells[i, j] = 1 - grid.cells[i, j]  # Inverser l'état actuel de la cellule
            appli.draw()
            t3 = time.time()
